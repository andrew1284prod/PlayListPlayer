...
# Список пакетов адаптирован под названия в репозиториях Debian/Ubuntu
...
def check_and_install_deps():
    ...
    # Обновляем кэш перед проверкой (опционально, но желательно)
    print(f"Обновление списков пакетов...")
    
    for pkg in REQUIRED_PACKAGES:
        # Проверка через dpkg-query (Debian/Ubuntu)
...
def select_directory_native():
    ...
    return input("Введите путь вручную (например, /home/user/playlistplayer): ").strip()

def run_setup():
    m = MSG[LANG]
    print(m["start"])

    # 1. Проверка пакетов через apt
    print(m["check_pkg"])
    ...
    try:
        ...
        # Файлы для копирования согласно проекту
        ...
        print("-" * 40)
        print(m["done"])
        print(f"[!] Теперь команды 'playlist', 'playlistconfig' и 'playlistupd' доступны.")
